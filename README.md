...
本拡張機能は、バラバラのファイルを一つの連続した「ストリーム」として再構築することで、視覚的なノイズを最小限に抑え、深い思考を中断することなく大規模なドキュメントセットを走査することを可能にします。
...
2. 右クリックして `Markdown Concat Viewer: Open` を実行します。
...
- `yarn compile`: 型チェック + Lint + ビルド
- `yarn watch`: 開発用ウォッチ（esbuild + tsc）
- `yarn package`: 本番向けビルド
- `yarn test`: テスト実行（VS Code Test）

### デバッグ実行

1. 依存関係をインストールして `yarn compile` を実行
2. VS Code でこの拡張プロジェクトを開く
3. `F5` で Extension Development Host を起動
4. 開発ホスト側のエクスプローラーで対象を選択してコマンド実行

## リリースノート

変更履歴は `CHANGELOG.md` を参照してください。

## GitHub Actions で VSIX リリース

このリポジトリには、タグ push を契機に VSIX をビルドして GitHub Release を作成するワークフローを用意しています。

- ワークフロー: `.github/workflows/release-vsix.yml`
- トリガー: `v*` 形式のタグ push（例: `v0.0.1`）
- 実行内容:
...
タグ名と `package.json` の `version` は一致させる運用を推奨します。
