...
## 主な機能
- **シームレスな連結表示**: 選択した複数の Markdown ファイル（`.md` / `.markdown`）を一つのキャンバスに統合してプレビューします。
- **構造の把握**: ファイル単位の折りたたみ機能と右ペインの動的TOCにより、全体像を維持したまま閲覧できます。
- **編集への即時遷移**: TOC の各見出し行をホバーすると編集ボタンが表示され、対象見出しの行位置で Markdown を編集タブとして直接開けます。
- **保存時の自動反映**: 表示中の対象 Markdown を保存すると、Concat View が自動リロードされて最新内容を反映します。
- **タブ名の変更**: Concat View のタブを右クリックして、表示名を任意の名前に変更できます。
...
- `markdownConcatViewer.renameTab`: 表示中の Concat View タブ名を変更
...
| 設定ID | デフォルト | 説明 |
| :--- | :--- | :--- |
| `markdownConcatViewer.preview.fontSize` | `100` | 本文プレビューのフォントサイズ（**エディタフォントサイズに対する%**） |
| `markdownConcatViewer.preview.lineHeight` | `175` | 本文プレビューの行間（%） |
| `markdownConcatViewer.preview.maxWidth` | `40` | 本文の最大表示幅（文字数換算 `ch`） |
| `markdownConcatViewer.toc.fontSize` | `12` | TOC（目次）のフォントサイズ（px） |
| `markdownConcatViewer.toc.minWidthChars` | `20` | TOCの基準幅（文字数換算） |

### TOCの表示モード

TOCは画面幅やユーザー操作に応じて以下の3モードで表示されます。

1. **通常表示 (Expanded)**: 本文とTOCが並列表示されます。
2. **最小化表示 (Minimized)**: TOCの幅比率が画面の30%を超える場合に自動遷移します。最小化時のTOC幅は60pxです。
3. **オーバーレイ表示 (Overlay)**: TOCを表示したときの本文残幅が600px未満になる場合に、TOCが本文上に重なって表示されます。

トグルボタンで手動切り替えも可能で、状態はWebview再読み込み時に復元されます。
